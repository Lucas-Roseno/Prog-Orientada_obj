...
void processarArquivo(const string& nomeArquivo) {
    try {
        ...
        {
            throw runtime_error("Número de astronautas na matriz: " + to_string(contAstro) 
            + " é difirente do número de astronautas na descrição: " + to_string(estacao.getAstronautas().size()));
        }
...
int main() {
    string nomeArquivo = "arquivos_entrada/entrada4.txt";
